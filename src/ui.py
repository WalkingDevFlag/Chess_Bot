...
import os
import shutil
from typing import Optional, List, Callable, Dict, Any # Added Dict, Any
import threading
import numpy as np # For Perlin noise map
...
# Import Perlin helper and input_automation
from perlin_noise_helpers import Perlin
import input_automation # For initializing globals
...
class ChessApp(ctk.CTk):
    def __init__(self):
        ...
        # Initialize attributes that will be created in _setup_ui to None or default
        self.output_textbox: Optional[ctk.CTkTextbox] = None
        self.debug_log_textbox: Optional[ctk.CTkTextbox] = None
        self.debug_log_textbox_frame: Optional[ctk.CTkFrame] = None # Added this for consistency

        # Call _setup_ui first to ensure all UI elements are created
        self._setup_ui()
        ...
        if PERLIN_ENABLED:
            # Now it's safe to call this, as UI elements for logging exist
    ...
    def _initialize_perlin_noise_system(self):
        ...
        try:
            ...
            self.add_to_output(f"Generating {noise_map_w}x{noise_map_h} Perlin noise map (scale: {PERLIN_NOISE_SCALE})...", "debug")
            start_time = time.time()
            self.global_perlin_noise_map = self.perlin_instance.noise_array(
                w=noise_map_w,
                h=noise_map_h,
                scale=PERLIN_NOISE_SCALE
            )
            gen_time = time.time() - start_time
            self.add_to_output(f"Perlin noise map generated in {gen_time:.2f}s.", "user")

            self.perlin_configs = {
                "noise_scale": PERLIN_NOISE_SCALE,
                "res_scale": PERLIN_RES_SCALE,
                "speed_min": PERLIN_SPEED_MIN,
                "speed_max_mul": PERLIN_SPEED_MAX_MUL,
                "jitter_mul": PERLIN_JITTER_MUL,
                "dev_deg": PERLIN_DEV_DEG,
                "sleep_interval": PERLIN_SLEEP_INTERVAL,
                "min_dist_sq": PERLIN_MIN_DIST_SQ
            }
            input_automation.init_perlin_globals(
                self.global_perlin_noise_map,
                self.perlin_configs,
                self.add_to_output
            )
            self.add_to_output("Perlin noise system ready.", "user")
        except Exception as e:
            self.add_to_output(f"Error initializing Perlin noise system: {e}", "user")
            self.global_perlin_noise_map = None
            self.perlin_configs = None
    ...
    def _setup_ui(self) -> None:
        ...
        # Assign to self.debug_log_textbox_frame
        self.debug_log_textbox_frame = ctk.CTkFrame(self.main_frame)
        # Assign to self.debug_log_textbox
        ...
        # Initially hide the debug log frame
        # self.debug_log_textbox_frame.pack_forget() # Already not packed by default

    def _toggle_debug_logs_command_handler(self) -> None:
        if not self.debug_log_textbox_frame : return # Should not happen
    ...
    def add_to_output(self, message: str, log_type: str = "user") -> None:
        # Ensure textboxes are initialized before trying to use them
        if self.output_textbox is None or self.debug_log_textbox is None:
            print(f"LOG ({log_type.upper()}): {message} (UI not fully initialized for logging)")
            return

        formatted_message = f"{message}\n"
        target_textbox = self.output_textbox if log_type == "user" else self.debug_log_textbox

        # Additional check for safety, though ideally _setup_ui ensures these are CTkTextbox instances
        if target_textbox is None:
             print(f"ERROR: Target textbox is None for log_type '{log_type}'. Message: {message}")
             return

        def _update_textbox():
            if target_textbox: # Check again inside closure
        ...
        # self.update_idletasks() # Can sometimes cause issues if called too rapidly from threads

    def _clear_output_command_handler(self) -> None:
        textboxes_to_clear = []
        if self.output_textbox: textboxes_to_clear.append(self.output_textbox)
        if self.debug_log_textbox: textboxes_to_clear.append(self.debug_log_textbox)

        for tb in textboxes_to_clear:
    ...
    def _update_internal_board_state(self) -> bool:
        ...
        if parsed_count == len(scraped_moves) and parsed_count > 0 :
            ...
            self.add_to_output("All scraped moves failed parsing.", "user"); return False
           ...
           self.engine_communicator.engine_process.poll() is None:
            try:
                ...
                if best_move_uci and best_move_uci != "(none)":
                    ...
                    except Exception as e: self.add_to_output(f"Suggested (UCI): {best_move_uci} (SAN parse error: {e})", "user")
    ...
    def _toggle_auto_play_mode_handler(self, mode: str) -> None:
        ...
        if PERLIN_ENABLED and (input_automation.GLOBAL_PERLIN_NOISE_MAP is None or \
                               input_automation.PERLIN_CONFIGS is None or \
                               input_automation.GLOBAL_LOGGER is None):
            self.add_to_output("Perlin noise system not ready. Attempting re-initialization.", "user")
            self._initialize_perlin_noise_system()
            if PERLIN_ENABLED and input_automation.GLOBAL_PERLIN_NOISE_MAP is None :
                 self.add_to_output("Re-init of Perlin failed. Auto-play aborted.", "user")
                 messagebox.showerror("Perlin Error", "Perlin noise system could not be initialized. Auto-play cannot start.")
                 return
